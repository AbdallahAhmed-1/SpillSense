...
from agents.mat_loader import MATLoader
from agents.mat_cleaner import MATCleaner
from agents.mat_explorator import MATExplorator
...
def build_hsi_graph():
    ...
    loader = MATLoader()
    cleaner = MATCleaner()
    explorator = MATExplorator(llm=llm)
    # Use reporter.run as the callable, not the class instance
    reporter_agent = Reporter(llm=llm, mode="mat")
